...
mixed effects linear models. This package however fits mixed 
effects models at individual gene level (using the `negative.binomial` family by default) and the `glmer` function in 
...
Or using glm only, as opposed to glmer:

```{r glmonly, warning=FALSE}
results_glm <- glmmSeq(modelFormula = ~ Timepoint * EULAR_6m,
                  countdata = tpm,
                  metadata = metadata,
                  dispersion = disp,
                  sizeFactors = NULL,
...
Additionally we can change the glm family: 

```{r poisson, warning=FALSE}
results_poisson <- glmmSeq(~ Timepoint * EULAR_6m + (1 | PATID),
                  id = "PATID",
                  countdata = tpm,
                  metadata = metadata,
                  dispersion = NA,
...
p1 <- pairedPlot(glmmResult=results_poisson,
           geneName = "IGHV3-23",
           x1Label = "Timepoint",
           x2Label="EULAR_6m",
           xTitle="Time",
           ...
           logTransform=TRUE,
           addViolin = TRUE,
           pairedOnly = FALSE) + labs(title="Poisson", y="IGHV3-23")
...
p2 <- pairedPlot(glmmResult=results,
           ...
           logTransform=TRUE,
           addViolin = TRUE,
           pairedOnly = FALSE) + labs(title="Negative Binomial", y="IGHV3-23")
...
Alternatively to plot the model fits alone you can use the modelPlot function. This also allows us to compare glm and glmer fits directly:
...
p1 <- modelPlot(results,
                ...
                graphics="ggplot",
                colours = plotColours) + 
  labs(title="ADAM12 with random effects (Chi-Sq stats)") + 
  lims(y=c(0, 12.5))

p2 <- modelPlot(results_glm,
                "ADAM12",
                x1Label="Timepoint",
                x2Label="EULAR_6m",
                xTitle="Time",
                ...
                graphics="ggplot",
                colours = plotColours) + 
  labs(title="ADAM12 without random effects (t-stats)") + 
  lims(y=c(0, 12.5))
...
2. [stats](https://www.R-project.org/): R Core Team (2020). R: A language and environment for statistical computing. R Foundation for Statistical Computing, Vienna, Austria.

3. [car](https://socialsciences.mcmaster.ca/jfox/Books/Companion/): John Fox and Sanford Weisberg (2019). An {R} Companion to Applied Regression, Third Edition. Thousand Oaks CA: Sage. 
...
5. [qvalue](https://github.com/StoreyLab/qvalue): John D. Storey, Andrew J. Bass, Alan Dabney and David Robinson (2020). qvalue: Q-value estimation for false discovery rate control. R package version 2.22.0. 